...
public class DatabaseController extends SQLiteOpenHelper
{
	...
	*/
	...
	Cursor searchContactByName(String myName)
	{
		
		SQLiteDatabase db = this.getReadableDatabase();
		Cursor cur = db.rawQuery("SELECT _ContactsID FROM Contacts WHERE NAME='"+myName+"'",null);
		cur.moveToFirst();
		int myID = -1;
		myID = (int) cur.getInt(cur.getColumnIndex("_ContactsID"));
		if(myID == -1)
		{
			return cur;
		}
		...
		{
			cur = db.rawQuery("SELECT * FROM Contacts WHERE _ContactID='"+myID+"'", null);
			return cur;			
		}
	}
